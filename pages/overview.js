...
import { getEvents, getPosts, getFeaturedMedia } from '../utils/wordpress';

import Post from '../components/Post';
import Event from '../components/Event';

export default function Overview({ posts, events }) {
  const jsxPosts = posts.map((post) => {
    ...
    //   }
    // } = post;
    // console.log(post['_embedded']['wp:featuredmedia'][0]['media_details']['file']);
    // const featuredMedia = `${MEDIA_URL}/${file}`;
    const featuredMedia = `${MEDIA_URL}/${post['_embedded']['wp:featuredmedia'][0]['media_details']['file']}`;
    // const featuredMedia = getFeaturedMedia(post.slug);
    return <Post {...post} featuredMedia={featuredMedia} key={post.id} />;
  });
